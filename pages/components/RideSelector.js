...
const RideSelector = (props) => {
    ...
    const getDirections = (pickUpCoordinates, dropoffCoordinates) => {
        fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${pickUpCoordinates[0]},${pickUpCoordinates[1]};${dropoffCoordinates[0]},${dropoffCoordinates[1]}?` + 
            new URLSearchParams({
                access_token: "pk.eyJ1IjoidGF0aGFnYXRhMTgwNSIsImEiOiJja3N2eXhkMDMxdWxuMnduMWxzc3JicnRrIn0.bdK-p33bBGeU0INC655enw",
            })
        )
        .then((response)=>{
            ...
            <CarList>
                { carList.map((car, index)=>(
                    ...
                    >
                        ...
                        <Price>INR {(duration/1000*car.multiplier).toFixed(2)}</Price>
                    </Car>
                )) }
            </CarList>
        </Wrapper>
